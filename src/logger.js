...
var logger = (function_ = console.log) => {
    return async (c, next) => {
        ...
        let start = Date.now();
        if (shouldLog()) {
            log(function_, `<-- (Incoming) FASTLY_SERVICE_VERSION: ${env('FASTLY_SERVICE_VERSION') || 'local'}` /* Incoming */, method, url);
            await next();
        } else {
            await next();
        }
        ...
        if (end > 10 || shouldLog() || c.error) {
            log(function_, `--> (Outgoing) FASTLY_SERVICE_VERSION: ${env('FASTLY_SERVICE_VERSION') || 'local'}` /* Outgoing */, method, url, c.res.status, time(start));
        }
    };
};
